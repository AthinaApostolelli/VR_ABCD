...
def get_lm_entry_exit(session, positions):
    ...
    if session['num_laps'] > 1:
        ...
        if (positions[0] - session['landmarks'][-1,1]) < (positions[0] - session['landmarks'][0,0]):
            search_start = np.where(positions >= session['all_landmarks'][0,0])[0][0]  # the mouse accidentally moved backwards first
        
        for lm_start in session['all_landmarks'][:,0]:
            lm_entry_idx.append(np.where(positions[search_start:] >= lm_start)[0][0])
        ...
        for lm_end in session['all_landmarks'][:,1]:
            ...
            lm_exit_idx.append(np.where(positions[search_start:] <= lm_end)[0][-1])
