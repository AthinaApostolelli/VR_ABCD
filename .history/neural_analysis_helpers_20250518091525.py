...
def get_lm_entry_exit(session, positions):
    ...
    if session['num_laps'] > 1:
        ...
        for i, (lm_start, lm_end) in enumerate(session['all_landmarks']):  # TODO: start iteration one lm later 
            ...
            search_start_lm = np.where(positions[search_start:] <= lm_start)[0][-1]

            # first_lm_end = np.where(positions[search_start:] <= lm_end)[0][-1] + search_start
            # print(first_lm_end)
            idx_candidates = np.where((positions[search_start_lm:] >= lm_start) & (positions[search_start_lm:] <= lm_end))[0]
