...
def plot_landmark_psth_map(average_psth, session, zscoring=True, sorting_lm=0, num_landmarks=10, time_around=1, funcimg_frame_rate=45, save_plot=False, savepath='', savedir='', filename=''):
    ...
    fig, ax = plt.subplots(1, num_landmarks, figsize=(num_landmarks*1.5+1,3), sharey=True, sharex=True, constrained_layout=True)
    ax = ax.ravel()
    ...
    for i in range(num_landmarks):
        img = ax[i].imshow(data[sortidx, i, :], aspect='auto')
    ...
    cbar.set_label('Z-scored Activity' if zscoring else 'Activity')
    plt.tight_layout()
