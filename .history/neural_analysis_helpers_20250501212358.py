...
def get_landmark_id_psth(data, neurons, event_idx, goals_idx, num_landmarks=2, time_around=1, funcimg_frame_rate=45):
    '''This function is similar to get_psth, but the average PSTH is calculated for each landmark separately.'''
    ...
    for i in range(num_landmarks):
        if i == 0:
            average_landmark_psth[:, i, :] = np.mean(psth[:, goals_idx, :], axis=1)
        elif i == 1:
            non_goals_idx = np.setdiff1d(np.arange(0, len(event_idx)), goals_idx)
            average_landmark_psth[:, i, :] = np.mean(psth[:, non_goals_idx, :], axis=1)
...
def plot_landmark_psth_map(average_psth, zscoring=True, sorting_lm=0, num_landmarks=10, time_around=1, funcimg_frame_rate=45, save_plot=False, savepath='', savedir='', filename=''):
    ...
    fig, ax = plt.subplots(1, num_landmarks, figsize=(num_landmarks*1.5+1,3), sharey=True, sharex=True)
    ax = ax.ravel()
