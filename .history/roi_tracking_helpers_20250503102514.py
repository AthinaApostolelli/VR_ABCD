...
def roicat_visualize_rois(roicat_dir, roicat_data_name, sessions_to_align, basepath, animal):
    '''Visualize the ROI alignment. For more details look at the ROICaT documentation https://roicat.readthedocs.io/en/latest/index.html.'''
    ...
    FOV_clusters = roicat.visualization.compute_colored_FOV(
        spatialFootprints=[r.power(1.0) for r in ROICaT_results['ROIs']['ROIs_aligned']],  ## Spatial footprint sparse arrays
        FOV_height=ROICaT_results['ROIs']['frame_height'],
        FOV_width=ROICaT_results['ROIs']['frame_width'],
        labels=ROICaT_results["clusters"]["labels_bySession"]
    )
