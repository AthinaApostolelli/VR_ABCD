...
def get_landmark_categories(sequence, num_landmarks, session):
    ...
    if num_landmarks == 10:     # T5 and T6
        ...
        non_goal_landmark_id = np.setdiff1d(np.arange(0, len(session['all_lms'])), goal_landmark_id)
    ...
    test_idx = np.where(np.isin(session['all_lms'], test_landmark_id))[0] if test_landmark_id is not None else None
