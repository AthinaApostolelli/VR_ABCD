...
class RichFileNode:
    ...
    def load(self):
        if self.loader:
            ...
            raise ValueError(f"No loader defined for {self.path}")

    def __repr__(self):
        return f"<RichFileNode path='{self.path}'>"

class RichFile_ROICaT:
    def __init__(self, path: Union[str, Path]):
        self.path = Path(path)
        self._tree = self._build_tree(self.path)
    ...
    def _build_tree(self, root: Path):
        ...
        for item in root.iterdir():
            if item.is_dir():
                ...
                if suffix == ".json":
                    loader = lambda p: json.load(open(p))
                elif suffix == ".npy":
                    loader = lambda p: np.load(p, allow_pickle=True)
                elif suffix == ".npz":
                    loader = lambda p: dict(np.load(p, allow_pickle=True))
                else:
                    loader = None
                tree[item.stem] = RichFileNode(item, loader=loader)
        return tree
    ...
    def __repr__(self):
        return f"<RichFile_ROICaT at '{self.path}'>"
