...
def get_lms_visited(options, session):
    ...
    if len(session['landmarks']) == 10:
        num_lms = len(session['landmarks'])*(session['num_laps']-1) + last_landmark # disregard last lap 
    else:
        num_lms = len(session['landmarks'])*(session['num_laps']) + last_landmark
