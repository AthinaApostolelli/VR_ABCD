...
def visualize_clusters_across_sessions(results):
    ucids = np.array(results['clusters']['labels'])
...
def visualize_output(results=None, roicat_dir=None, roicat_data_name=None, dir_save=None, 
                     plot_metrics=True, plot_clusters=True, plot_clusters_across_sessions=True,
                     plot_aligned_tracked_clusters=True):
    ...
    # Plot number of clusters across sessions 
    if plot_clusters_across_sessions:
        _ = visualize_clusters_across_sessions(results)
...
def align_rois(roicat_dir, roicat_data_name, sessions_to_align, basepath, animal, alignment_method='F', 
               neural_data=None, roicat_results=None, plot_alignment=False, save_results=False, 
    ...
    if os.path.exists(os.path.join(alignment_dir, filename)):
        ...
        if neural_data is None:
            neural_data = load_neural_data(basepath, animal, sessions_to_align, alignment_method=alignment_method)
...
# TODO: find the neurons tracked across all sessions 

# TODO: find the neurons tracked session by sessions 
